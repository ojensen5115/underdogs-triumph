...
function ShowHideNav() {
    ...
    $header = $(".header"),
        // just storing header in a variable
    navHeight = $header.outerHeight(),
        // nav height
    detachPoint = 576 + 60,
        // after scroll past this nav will be hidden
...
function displayShoppingCart(cartDisplay) {
    ...
    for (var i = 0; i < cart.items.length; i++) {
        ...
        var quantityNum = $('<input type="number">').attr('data-name', cart.items[i].name).val(cart.items[i].quantity).change(function (e) {
            var name = $(this).attr('data-name');
            var cart = getCart();
            for (var i = 0; i < cart.items.length; i++) {
        ...
        var row = $('<tr>').append($('<td>').append('<img src="https://placekitten.com/100/100">')).append($('<td>').text(cart.items[i].name)).append($('<td>').text(money(cart.items[i].price))).append(quantityCell);
